...
func SaveOTP(ctx context.Context, db *sql.DB, identifier, otp string) error {
	...
	if err != nil {
		...
		// Log security event for OTP save failure
		monitoring.LogAuthFailure(ctx, identifier, "", "", map[string]interface{}{
	...
	// Log security event for OTP save success
	monitoring.LogAuthSuccess(ctx, "", identifier, "", "", map[string]interface{}{
...
func VerifyAndDeleteOTP(ctx context.Context, db *sql.DB, identifier, otp string) (bool, error) {
	...
	if err != nil {
		...
		// Log security event for OTP verification failure
		monitoring.LogAuthFailure(ctx, identifier, "", "", map[string]interface{}{
