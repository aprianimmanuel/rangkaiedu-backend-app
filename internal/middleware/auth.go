...
func AuthRequired() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if authHeader == "" {
			// Log security event for missing authorization header
			monitoring.LogAuthViolation(c.Request.Context(), "", "", c.Request.URL.Path, "MISSING_AUTH_HEADER", map[string]interface{}{
				"ip": c.ClientIP(),
			})
		...
		if tokenString == authHeader {
			// Log security event for invalid authorization header format
			monitoring.LogAuthViolation(c.Request.Context(), "", "", c.Request.URL.Path, "INVALID_AUTH_FORMAT", map[string]interface{}{
				"ip": c.ClientIP(),
			})
		...
		if err != nil {
			...
			monitoring.LogAuthViolation(c.Request.Context(), "", "", c.Request.URL.Path, "INVALID_TOKEN", map[string]interface{}{
				"ip": c.ClientIP(),
				"error": err.Error(),
			})
